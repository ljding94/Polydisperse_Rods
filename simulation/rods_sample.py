...
def run_sampling(save_dump_detail, system_params, q_values, measurement_steps=10000, N_measurement=100):
    ...
    num_types, particle_types, type_Ls, type_Ds, type_Vs, total_V, total_V2 = read_shape_info(subfolder)
    ...
    for i in range(num_types):
        ...
        mc.shape[type_name] = dict(vertices=vertices, sweep_radius=0.5 * D, ignore_statistics=False)  # tell mc integrator the size of each type
    ...
    dumper = DumpTXT(subfolder, type_Ls, type_Ds, type_Vs, "sampling")
    if save_dump_detail:
    ...
    subfolder = f"{folder}/{label}"
    Iq_action = MeasureScattering(folder, subfolder, q_values, particle_types, type_Ls, type_Ds, type_Vs, system_params, "sample")
